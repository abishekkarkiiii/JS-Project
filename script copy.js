    ...
    for (let row = 1; row <= 1; row++) {
        ...
        for (let col = 1; col <= columns; col++) {
            ...
            colDiv.onclick = function () {
                ...
                // Check for game over condition with a delay
                setTimeout(() => {
                    if (getRandomNumber(columns) === 1) {
...
function incrementLevel() {
  if(gameMode=="normal"){
    gameMode="medium";
    createContainer(levels[gameMode][0]+levels[gameMode][1]);
    ...
    createContainer(levels[gameMode][0]+levels[gameMode][1]);  }
        ...
        alert("Congratulationsss YOu Won the Game! Your score: " + score);
    }
}
